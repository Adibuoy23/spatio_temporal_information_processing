...
print(os.getcwd())
os.chdir(os.path.join(os.getcwd(),'/Volumes/My Passport/Github/spatio_temporal_information_processing/Rate_Subjective_Expansion'))
        ...
        for key in event.getKeys():  # check if pressed abort-type key
            ...
            if key in ['s', 'l', 'escape']:
                ...
                if dur <= Durations and key in ['s']:
                    response.append({'Correct':1, 'oddBallDur':dur, 'standardStimDur':Durations, 'keyPress': key, 'eccentricity': eccentricity[ix]})
                elif dur > Durations and key in ['l']:
                    response.append({'Correct':1, 'oddBallDur':dur, 'standardStimDur':Durations, 'keyPress': key, 'eccentricity':eccentricity[ix]})
                else:
                    response.append({'Correct':0, 'oddBallDur':dur, 'standardStimDur':Durations, 'keyPress': key, 'eccentricity': eccentricity[ix]})
            if key in ['escape']:
                expStop = True
                core.quit()
                myWin.clearBuffer()
