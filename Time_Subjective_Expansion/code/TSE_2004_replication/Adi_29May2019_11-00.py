...
possibleStandardStimLoc = np.repeat([7,8,9,9,10,11],5)  # repeat this 10 times
shuffle(possibleStandardStimLoc)
presentationSqeuence = []
for stimLoc in possibleStandardStimLoc:
    presentationSqeuence.extend(np.repeat(1,stimLoc)) # Flag for standard stimuli (value = 1 is standard)
    presentationSqeuence.append(0) #Flag for  oddball (valu = 0 is odd ball. used later in the code)
possibleOddballDurations = np.repeat([450, 525, 600, 675, 750, 825, 900, 975, 1050, 1125],3) # total 60
shuffle(possibleOddballDurations)

zero_index = [i for i, e in enumerate(presentationSqeuence) if e == 0]
Durations = list(np.repeat(1050,600)) # Create a list of durations with 1050msec

for i,z in enumerate(zero_index):
    Durations[z] = possibleOddballDurations[i] #Insert oddball durations in the list according to the presentationSqeuence created above. (Insert oddball duration wherever presentationSqeuence is zero)
...
oddBallStim = visual.Circle(myWin,
                    radius=oddBallMaxRadius,  # Martini used circles with diameter of 12 deg
                    lineColorSpace='rgb',
                    lineColor='red',
                    lineWidth=2.0,  # in pixels
                    units='deg',
...
for i,type in enumerate(presentationSqeuence):
    if type: #standard stimulus
        standardStim.setAutoDraw(True)
        myWin.flip()
        core.wait(Durations[i]) #Standard stimuli for a duration of 1050msec
        ...
        myWin.flip()
        oddBallClock.reset()
        expansionRate = (oddBallMaxRadius - oddBallMinRadius)/Durations[i]
        counter = i
        core.wait(Durations[i])
        #while oddBallClock.getTime() < Durations[i]:
        ...
        while waiting:
            ...
            for key in event.getKeys():  # check if pressed abort-type key
                ...
                if key in ['s', 'l', 'escape']:
                    ...
                    myWin.flip()
                    if Durations[counter] <= Durations[counter-1] and key in ['s']:
                        response.append({'Correct':1, 'oddBallDur':Durations[counter], 'standardStimDur':Durations[counter-1]})
                    elif Durations[counter] > Durations[counter-1] and key in ['l']:
                        response.append({'Correct':1, 'oddBallDur':Durations[counter], 'standardStimDur':Durations[counter-1]})
                    else:
                        response.append({'Correct':0, 'oddBallDur':Durations[counter], 'standardStimDur':Durations[counter-1]})
                if key in ['escape']:
                    expStop = True
                    core.quit()
                    myWin.clearBuffer()
