...
fixation = visual.Circle(myWin,
                    radius=oddBallMinRadius/2,  # Martini used circles with diameter of 12 deg
                    lineColorSpace='rgb',
                    lineColor='red',
                    lineWidth=2.0,  # in pixels
                    units='deg',
...
for dur in possibleOddballDurations:
    for num in range(1, randint(7,12)):
        ...
        standardStim.setPos([3*np.cos(theta),3*np.sin(theta)])
    ...
    theta = random.uniform(0,359)*np.pi/180
    oddBallStim.setPos([3*np.cos(theta),3*np.sin(theta)])
