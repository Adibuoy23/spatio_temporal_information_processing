...
INS_MSG2 = "We really want you to be as accurate as you can. So a game will be played to make it more fun. We want you to hold down the space bar for as long as you saw the stanrd disc last, but no longer! The closer you get to the right answer, the more points we will award you in that trial. But if you go over, your disc will disappear on its own and you won't get any points on that trial. So you need to be as accurate as possible without going over to earn the max. If you collect a total of XXX points by the end, we will double the credit you get for the experiment.\n\n"
INS_MSG2 += "If you're feeling uncomfortable, you can press ESC key any time to stop the experiment.\n\n"
INS_MSG2 += "Press any key you are ready to begin the experiment.\n\n"
...
print(possibleOddballDurations, type)
        ...
        if oddBallClock.getTime() > dur/1000 and 'space' in keys:
            ...
            if 'space' in keys:
                for key in keys:
                    ...
                    judgment.setAutoDraw(False)
                    myWin.flip()
                    between_trial(myWin, display_text, "Duration pressed : " +str(np.round(key.duration,2))+", Actual:"+str(dur/1000)+ ".\n\n You earned "+str(current_points)+" points in this round.\n\n Press [SPACE] to advance")
                    reward_text.setText("Total points : "+str(np.round(reward_counter,2)))
