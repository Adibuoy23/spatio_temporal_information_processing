...
for ix,dur in enumerate(possibleOddballDurations):
    ...
    while  time <= dur/1000:
        ...
        if 'space' in keys and time <= dur/1000:
            for key in keys:
                fixation.setAutoDraw(False)
                display_text.setAutoDraw(True)
                display_text.setText('Your response : ' + str(np.round(key.rt,2)))
                response.append({'Duration':dur, 'Response':key.rt, 'Each condition':ix})
                myWin.flip()
            waiting=False
        elif 'space' in keys and time > dur/1000:
            fixation.setAutoDraw(False)
            display_text.setAutoDraw(True)
            display_text.setText('Your response : ' + str(np.round(key.rt,2)))
            response.append({'Duration':dur, 'Response':key.rt, 'Each condition':ix})
