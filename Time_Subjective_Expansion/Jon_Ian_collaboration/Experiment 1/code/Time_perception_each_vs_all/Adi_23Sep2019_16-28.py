...
for ix,dur in enumerate(possibleOddballDurations):
    ...
    while waiting:
        ...
        if oddBallClock.getTime() > dur/1000 and 'space' in keys:
            ...
            judgment.setAutoDraw(False)
            myWin.flip()
            response.append({'Duration':dur/1000, 'Response':keys[0].duration, 'Each condition':ix})
            oddBallClock.reset()
            kb.clock.reset()
        else:
            if 'escape' in keys:
