...
possibleOddballDurations = np.repeat(
...
oddBallMaxRadius = 1.06
oddBallMaxRadius1 = 1.06
...
for ix, dur in enumerate(practiceDurations):
    ...
    myWin.flip()
    if practiceType[ix] == 1:
        while oddBallClock.getTime() <= dur/1000:
            if oddBallClock.getTime() >= onset[1]*dur/1000:
                judgment.setColor('blue')
            myWin.flip()
    ...
    kb.start()
    waiting = True
    between_trial(myWin, display_text,
    ...
    while waiting:
        ...
        if oddBallClock.getTime() > dur/1000 and 'space' in keys:
            ...
            kb.stop()
            between_trial(myWin, display_text,
                          "Your response exceeded the actual time.\n\n Press [SPACE] to advance")
            myWin.flip()
            oddBallClock.reset()
            kb.clock.reset()
        else:
            ...
            if 'space' in keys:
                for key in keys:
                    ...
                    kb.stop()
                    myWin.flip()
                    between_trial(myWin, display_text, "You earned "+str(np.round(current_points, 2)
                                                                         )+" out of 10 points in this round.\n\n Press [SPACE] to advance")
                    reward_text.setText("Total points : "+str(np.round(reward_counter, 2)))
...
for ix, dur in enumerate(possibleOddballDurations):
    ...
    while waiting:
        ...
        if oddBallClock.getTime() > dur/1000 and 'space' in keys:
            ...
            myWin.flip()
            response.append(
                {'Duration': dur/1000, 'Response': keys[0].duration, 'Stimulus condition': types[ix]})
            oddBallClock.reset()
            kb.clock.reset()
        else:
            if 'escape' in keys:
            ...
            if 'space' in keys:
                for key in keys:
                    response.append({'Duration': dur/1000, 'Response': key.duration,
                                     'Stimulus condition': types[ix], 'OddBall Onset': oddnessOnsetRatio[ix]})
                    diff_time = np.abs(key.duration-dur/1000)
                    current_points = np.round((np.exp(-diff_time*2)), 2)*10
                    reward_counter += current_points
                    kb.stop()
