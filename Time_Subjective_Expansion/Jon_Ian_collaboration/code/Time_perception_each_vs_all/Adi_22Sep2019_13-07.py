...
for ix,dur in enumerate(possibleOddballDurations):
    ...
    while waiting:
        judgment.draw()
        myWin.flip()
        keys = kb.getKeys(['space', 'escape'], waitRelease=True, bufferSize=2)
        if oddBallClock.getTime() > dur/1000 and 'escape' not in keys:
            print(oddBallClock.getTime())
            waiting = False
            ...
            if 'space' in keys:
                for key in keys:
                    ...
                    display_message(myWin, display_text, "Duration pressed : " +str(np.round(key.duration,2))+".\n\n You earned "+str(current_points)+" points in this round.\n\n Press any key to advance")
                    reward_text.setText("Total points : "+str(reward_counter))
                    myWin.flip()
                waiting=False
    kb.stop()
