...
for ix,dur in enumerate(possibleOddballDurations):
    ...
    response = event.waitKeys(timeStamped=oddBallClock, maxWait=dur/1000)

    if response:
        if 'escape' in response[0][0]:
            core.quit()
            win.close()
        elif 'space' in response[0][0]:
            display_text.setText('Your response: ' +str(response[0][1]) + ' sec.')
            response.append({'Duration':dur, 'Response':response[0][1], 'Each condition':ix})
            myWin.flip()
    else:
