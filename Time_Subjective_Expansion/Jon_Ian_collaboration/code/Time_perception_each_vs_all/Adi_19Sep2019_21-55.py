...
for ix,dur in enumerate(possibleOddballDurations):
    display_message(myWin, display_text, 'Press right arrow to advance')
    fixation.setAutoDraw(True)
    myWin.flip()
    core.wait(1)
    ...
    display_text.setText("Get ready for the stimulus to appear. Your task is to press [SPACE] exactly when you think the stimulus is going to disappear")
    myWin.flip()
    ...
    keys = event.waitKeys(timeStamped=oddBallClock, maxWait=dur/1000)

    if keys:
        if 'escape' in keys[0][0]:
            core.quit()
            win.close()
        elif 'space' in keys[0][0]:
            diff_time = np.abs(keys[0][1]-dur/1000)
            current_points = int(10* np.exp(-0.5*diff_time))
            reward_counter+= current_points
            display_text.setAutoDraw(True)
            fixation.setAutoDraw(False)
            display_text.setText('You gained '+str(current_points)+' points.\n\n Total: '+str(reward_counter))
            response.append({'Duration':dur, 'Response':keys[0][1], 'Each condition':ix})
            oddBallStim2.setAutoDraw(False)
            myWin.flip()
    else:
        display_text.setAutoDraw(True)
        fixation.setAutoDraw(False)
        display_text.setText('Your response exceeded the true time.')
        response.append({'Duration':dur, 'Response':np.nan, 'Each condition':ix})
        oddBallStim2.setAutoDraw(False)
        myWin.flip()
    core.wait(0.7)
    display_text.setAutoDraw(False)
