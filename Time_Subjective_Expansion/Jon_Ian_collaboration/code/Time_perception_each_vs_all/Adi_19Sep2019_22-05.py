...
for ix,dur in enumerate(possibleOddballDurations):
    ...
    display_text.setText("Get ready to Press [SPACE] exactly when you think the stimulus is going to disappear")
    myWin.flip()
    ...
    if keys:
        if 'escape' in keys[0][0]:
            ...
            current_points = np.round((10* np.exp(-diff_time/8)),2)
