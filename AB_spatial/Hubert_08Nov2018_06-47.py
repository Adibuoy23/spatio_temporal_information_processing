...
if doStaircase:
    ...
    while nDoneMain < trials.nTotal and expStop == False:
        ...
        letterSequence, cuesPos, correctAnswers, ts = do_RSVP_stim(
            cue1pos, cue2lag, noisePercent/100., nDoneMain)
        ...
        tempResp = list(numberToLetter(letterSequence))
        print(tempResp)
        print(numberToLetter(correctAnswers))
        print(numberToLetter(responses))
        correctIndex = tempResp.index(numberToLetter(correctAnswers[0]))
        responseIndex = tempResp.index(numberToLetter(responses[0]))
        diffIndex = correctIndex - responseIndex
        correctResponseStim = visual.TextStim(
            myWin, text="The correct answer is : "+str(numberToLetter(correctAnswers[0]))+"\n It is "+str(diffIndex)+"away from the correct answer")
        event.waitKeys()
