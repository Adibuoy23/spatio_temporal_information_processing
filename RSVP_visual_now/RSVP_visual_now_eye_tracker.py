...
def oneFrameOfStim(n, cue, cueSpatialLoc, letterSequence, cueDurFrames, letterDurFrames, ISIframes, cuesPos, lettersDrawObjects,
                   noise, proportnNoise, allFieldCoords, numNoiseDots, eyeTracking):
    ...
    for cueFrame in cueFrames:  # cheTck whether it's time for any cue
        ...
        if n == cueFrame and eyeTracking:
            getEYELINK().sendMessage("Probe on")
        ...
        if eyeTracking:
            getEYELINK().sendMessage("Probe off")
...
def do_RSVP_stim(cue1pos, cueSpatialLoc, cue2lag, proportnNoise, trialN):
    ...
    if eyeTracking:
        getEYELINK().sendMessage("RSVP on")
    ...
    for n in range(trialDurFrames):  # this is the loop for this trial's stimulus!
        worked = oneFrameOfStim(n, cue, cueSpatialLoc, letterSequence, cueDurFrames, letterDurFrames, ISIframes, cuesPos, lettersDrawObjects,
                                noise, proportnNoise, allFieldCoords, numNoiseDots, eyeTracking)  # draw letter and possibly cue and noise on top
        if exportImages:
    ...
    if eyeTracking:
        getEYELINK().sendMessage("RSVP off")
...
blockNumber = 1
if eyeTracking:
    eyelinktracker, edfFileName = start_eyeTracking(
        eyeFileName+str(blockNumber), screen_width=widthPix, screen_height=heightPix, pixel_per_degree=pixelPerDegree)
if doStaircase:
    ...
    while nDoneMain < trials.nTotal and expStop == False:
        if eyeTracking and (not getEYELINK().isConnected() or getEYELINK().breakPressed()):
            eyeTracking_done()
            break

        if eyeTracking:
            msg = "TRIAL %d" % (nDoneMain)
            getEYELINK().sendMessage(msg+" starts")
            error = eyeTracking_ready(screen_width=widthPix, screen_height=heightPix)
            if error:
                eyeTracking_done()
                break
        ...
        if eyeTracking:
            getEYELINK().sendMessage(msg+" ends")
            eye_tracking_done()
                ...
                while waiting:
                    ...
                    for key in event.getKeys():  # check if pressed abort-type key
                        ...
                        if key in ['space']:
                            ...
                            if eyeTracking and blockNumber <= int(round(pctCompletedBreak/100)):
                                eyelinktracker, edfFileName = start_eyeTracking(
                                    eyeFileName+str(blockNumber), screen_width=widthPix, screen_height=heightPix, pixel_per_degree=pixelPerDegree)
                myWin.clearBuffer()
            core.wait(.2)
