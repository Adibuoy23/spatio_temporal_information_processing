    ...
    while not expStop and (numResponses < numCharsWanted or not accepted):
        ...
        while noResponseYet:  # loop until a valid key is hit
            ...
            if autopilot:
                ...
                # process only the last key, it being the most recent. In theory person could type more than one key between window flips,
                key = keysPressed[-1]
...
#                      noResponseYet = False
                    ...
                    # not just using len(responses) because want to work even when autopilot, where thisResponse is null
                    numResponses += 1
                    click = True
...
if __name__ == '__main__':  # Running this file directly, must want to test functions in this file
    ...
    respPromptStim = visual.TextStim(window, pos=(0, -.7), colorSpace='rgb', color=(
        1, 1, 1), alignHoriz='center', alignVert='center', height=.1, units='norm', autoLog=autoLogging)
    acceptTextStim = visual.TextStim(window, pos=(0, -.8), colorSpace='rgb', color=(
        1, 1, 1), alignHoriz='center', alignVert='center', height=.1, units='norm', autoLog=autoLogging)
    acceptTextStim.setText('Hit ENTER to accept. Backspace to edit')
    respStim = visual.TextStim(window, pos=(0, 0), colorSpace='rgb', color=(
    ...
    expStop, passThisTrial, responses, responsesAutopilot = \
        collectStringResponse(numCharsWanted, respPromptStim, respStim, acceptTextStim,
                              window, clickSound, badKeySound, requireAcceptance, autopilot, responseDebug=True)
