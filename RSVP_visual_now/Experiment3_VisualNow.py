...
widthPix = 5120/2  # monitor width in pixels of Agosta
heightPix = 2880/2  # 800 #monitor height in pixels
monitorwidth = 64.77  # 28.2  # monitor width in cm
...
INS_MSG += "You will perform this task while fixating in the center. \n\n"
INS_MSG += "The stream of letters is presented anywhere on the horizontal axis of the screen.\n\n"
INS_MSG += "The letter in the stream that you're supposed to identify is accompanied by a probe that can also appear anywhere on the horizontal axis of the screen.\n\n"
...
cueDurFrames = int(letterDurFrames/2)
...
if not os.path.exists(os.path.join(movieDir,expt_name)):
    os.makedirs(os.path.join(movieDir,expt_name))
fileName = os.path.join(dataDir, expt_name, subject + '_' + infix + timeAndDateStr)
movFname = os.path.join(movieDir, expt_name, subject + '_' + infix + timeAndDateStr)+'.jpg'
...
fixationPoint = visual.PatchStim(myWin, tex='none', colorSpace='rgb', color=(
    1, 0, 0), size=10, units='pix', autoLog=autoLogging)
...
def oneFrameOfStim(n, cue, cueSpatialLoc, letterSequence, cueDurFrames, letterDurFrames, ISIframes, cuesPos, lettersDrawObjects,
    ...
    ltrSpatialLoc = [-x for x in cueSpatialLoc]
    ...
    if showLetter:
        ...
        lettersDrawObjects[thisLetterIdx].setPos([0,0])
    ...
    while nDoneMain < len(trials) and expStop == False:
        ...
        if not expStop:
            ...
            if exportImages:  # catches one frame of response
                ...
                framesSaved += 1
                core.wait(.1)
                myWin.saveMovieFrames(movFname)
                expStop = True
            core.wait(.1)
            if feedback:
                play_high_tone_correct_low_incorrect(correct, passThisTrial=False)
