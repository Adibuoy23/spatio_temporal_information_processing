...
try:
    import stringResponse_color_change_ver2 as stringResponse
except ImportError:
...
def oneFrameOfStim(n, cueSpatialLoc, letterSequence, cueDurFrames, letterDurFrames, cuesPos, changeCue1):
    ...
    if n >= cueFrames[1] and n < cueFrames[1]+cueDurFrames:
        cue2.setLineColor('red')
        cue2.setFillColor('red')
    else:
        cue2.setLineColor('green')
        cue2.setFillColor('green')

    cue2.setPos(cueSpatialLoc)
    if changeCue1:
        cue1.setPos(-cueSpatialLoc)
    else:
        cue1.setPos((0, 0))
...
def do_RSVP_stim(cue1pos, cueSpatialLoc, cue2lag, trialN, changeCue1):
    ...
    letterSequence = np.arange(0, numLettersToPresent)
...
def handleAndScoreResponse(passThisTrial, responses, letterSequence, cuesPos, correctAnswers, cueSpatialPosition, cueEcc):
...
# print('trialnum\tsubject\t', file=dataFile, end='')
...
INS_MSG += "You will be presented with two discs, one in blue and the other in green colors.\n\n"
INS_MSG += "Your task is to say which colored disk flashed first.\n\n"
INS_MSG += "You will do this while fixating in the center.\n\n"
INS_MSG += "Once you've identified the order after the trial ends, type it out on the keyboard.\n\n"
...
SOAMs = 50  # Battelli, Agosta, Goodbourn, Holcombe mostly using 133
...
# text1 = visual.TextStim(myWin, pos=(0, 0), colorSpace='rgb', color=(1, 1, 1),
#                        alignHoriz='center', alignVert='center', height=.1, units='norm', autoLog=autoLogging)
# text1.setText('1')
# text2 = visual.TextStim(myWin, colorSpace='rgb', color=(1, 1, 1),
#                        alignHoriz='center', alignVert='center', height=.1, units='norm', autoLog=autoLogging)
# text2.setText('2')
...
possibleCue1positions = np.array([10])  # [4,10,16,22] used in Martini E2, group 2
cueCoords = [[1, 0], [-1, 0]]
cueEccentricity = [1.5, 3, 4.5, 9]
possibleCue2lags = np.array([-4, -3, -2, -1, 0, 1, 2, 3, 4])
for cue1pos in possibleCue1positions:
    for cue2lag in possibleCue2lags:
        for coords in cueCoords:
            for ix, ecc in enumerate(cueEccentricity):
                if ix % 2 == 0:
                    changeCue = True
                else:
                    changeCue = False
                stimList.append({'cue1pos': cue1pos, 'cue2lag': cue2lag,
                                 'cueCoords': coords, 'cueEccentricity': ecc, 'changeCue1': changeCue})
...
dataFile.write('cueSpatialPos\t')
dataFile.write('cueEccentricity\t')
...
while nDoneMain < trials.nTotal and expStop == False:
    ...
    changeCue1 = thisTrial['changeCue1']

    letterSequence, cuesPos, correctAnswers, ts = do_RSVP_stim(
        cue1pos, np.array(cueSpatialPosition)*cueEcc, cue2lag, nDoneMain, changeCue1)
    ...
    if not expStop:
        ...
        correct, eachCorrect, eachApproxCorrect, T1approxCorrect, passThisTrial, expStop = (
            handleAndScoreResponse(passThisTrial, responses, letterSequence, cuesPos, correctAnswers, cueSpatialPosition, cueEcc))
