...
def handleAndScoreResponse(passThisTrial, responses, letterSequence, cuesPos, correctAnswers, cueSpatialPosition, cueEcc):
    ...
    for i in range(len(cuesPos)):  # score response to each cue
        if correctAnswers[i] == responses[i]:
            eachCorrect[i] = 1
        posThisResponse = np.where(responses[i] == letterSequence)
    ...
    for i in range(len(cuesPos)):  # print response stuff to dataFile
        ...
        answerCharacter = letterSequence[cuesPos[i]]
...
text1.setText('1')
text2 = visual.TextStim(myWin, pos=(0, 0), colorSpace='rgb', color=(1, 1, 1),
                        alignHoriz='center', alignVert='center', height=.1, units='norm', autoLog=autoLogging)
text2.setText('2')
