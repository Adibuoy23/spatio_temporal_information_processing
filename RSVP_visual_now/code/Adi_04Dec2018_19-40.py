...
def oneFrameOfStim(n, cue, cueSpatialLoc, letterSequence, cueDurFrames, letterDurFrames, ISIframes, cuesPos):
    # defining a function to draw each frame of stim. So can call second time for tracking task response phase
    SOAframes = letterDurFrames+ISIframes
    cueFrames = cuesPos*SOAframes  # cuesPos is global variable

    if n >= cueFrames[0] and n < cueFrame[0]+cueDurFrames:
    ...
    if n >= cueFrames[1] and n < cueFrame[1]+cueDurFrames:
...
def do_RSVP_stim(cue1pos, cueSpatialLoc, cue2lag, trialN):
    ...
    for n in range(trialDurFrames):  # this is the loop for this trial's stimulus!
        worked = oneFrameOfStim(n, cue, cueSpatialLoc, letterSequence, cueDurFrames, letterDurFrames,
                                ISIframes, cuesPos)  # draw letter and possibly cue and noise on top
