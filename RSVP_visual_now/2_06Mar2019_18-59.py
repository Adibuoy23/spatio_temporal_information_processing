...
if os.path.isdir('.'+os.sep+'data'):
    ...
    expt_name = 'RSVP_two_streams_bigger_cue_size_different_rates'
else:
...
possibleCue1positions = np.array([7, 11, 15, 19, 23])  # [4,10,16,22] used in Martini E2, group 2
cueCoords = [[1, 0], [-1, 0]]
cueEccentricity = [3.0, 10.0]
...
for i in range(numRespsWanted):
    ...
    stimsInTrialsFile.write('cueEccentricity'+str(i+1)+'\t')
...
cue = visual.Circle(myWin,
                    ...
                    units='deg',
                    fillColorSpace='rgb',
                    fillColor=bgColor,  # beware, with convex shapes fill colors don't work
...
for i in range(0, 26):
    ...
    cueDraw.setText(cletter, log=False)
...
def oneFrameOfStim(n, cue, cueEcc, cueSpatialLoc, letterSequence, cueLetterSequence, cueDurFrames, letterDurFrames, ISIframes, cuesPos, lettersDrawObjects, cueDrawObjects,
    ...
    #cueDrawObjects[thisCueLetterIdx].setHeight(max(ltrHeight, 0.5 * cueEcc))
...
def do_RSVP_stim(cue1pos, cueEcc, cueSpatialLoc, cue2lag, proportnNoise, trialN):
    ...
    for i in cueLetterSequence[1::2]:
        cueDrawObjects[i].setText(' ')
    noise = None
    allFieldCoords = None
    numNoiseDots = 0
    ...
    for n in range(trialDurFrames):  # this is the loop for this trial's stimulus!
        worked = oneFrameOfStim(n, cue, cueEcc, cueSpatialLoc, letterSequence, cueLetterSequence, cueDurFrames, letterDurFrames, ISIframes, cuesPos, lettersDrawObjects, cueDrawObjects,
...
def handleAndScoreResponse(passThisTrial, responses, responsesAutopilot, task, letterSequence, cuesPos, correctAnswers, cueSpatialPosition, cueEcc):
    ...
    print(cueSpatialPosition, '\t', end='', file=stimsInTrialsFile)
    print(cueEcc, '\t', end='', file=dataFile)
    print(cueEcc, '\t', end='', file=stimsInTrialsFile)
...
if doStaircase:
    ...
    while nDoneMain < len(trials) and expStop == False:
        ...
        for i in range(26):
            ...
            cletter = numberToLetter(i)
            cueDrawObjects[i].setText(cletter, log=False)
        ...
        correctIndex = tempResp.index(correctAnswers[0])
        respIndex = tempResp.index(letterToNumber(responses[0]))
        diffIndex = correctIndex - respIndex
        correctResponseStim = visual.TextStim(
            myWin, text="The correct answer is : "+str(numberToLetter(correctAnswers[0]))+"\n It is "+str(diffIndex)+" lag away from the correct answer")
        event.waitKeys()
